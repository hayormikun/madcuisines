...
const createCategory = async(category: ICategory): Promise<ICategory> => {
  return await axios.post('http://api.madcuisines.com/category/create', category)
}
...
const create = () => {
  ...
  const { mutate, isLoading,isError, error, isSuccess }: UseMutationResult<ICategory, Error, ICategory> = useMutation<ICategory, Error, ICategory>(createCategory)
    ...
    <main className='lg:flex pt-20'>
      <Sidebar view='Categories' create='Category' viewLink='/category' createLink='/category/create' />
        <div className="mt-5 w-full lg:w-10/12">
          ...
          <div className="grid">
          { isError ? `Error encountered while creating Category: ${error.message}` : ''}
          { isSuccess ? 'Category created successfully' : '' }
              <form onSubmit={handleSubmit} action="POST" className="text-gray-700 font-semibold mx-auto w-6/12">
               ...
               { isLoading ? <WideButton name="Creating Category... "/> : <WideButton name="Create Category" /> }
